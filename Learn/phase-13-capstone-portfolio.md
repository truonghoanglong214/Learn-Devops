...
- [ ] Code có lint rules và formatter (`dotnet format`, Roslyn Analyzers, StyleCop.Analyzers cho .NET)
- [ ] Không có commented-out code thừa trong production branch
- [ ] Dependency versions được pin (`.csproj` với `<PackageReference Include="..." Version="x.x.x" />` cụ thể, không dùng floating versions `*`)
...
`.gitignore` cần được setup ngay từ commit đầu tiên, không phải sau khi đã lỡ commit file nhạy cảm. Template cơ bản cho project .NET full-stack:
...
Pipeline chậm là dấu hiệu team không quan tâm đến developer experience. Nếu CI mất 20 phút, developer sẽ bắt đầu push ít hơn và batch nhiều thay đổi vào 1 commit — ngược lại với best practice. Cách tối ưu pipeline cho .NET:

1. Cache NuGet packages giữa các run: `actions/cache` với key là hash của `*.csproj` files
2. Docker BuildKit với layer cache: `--cache-from` và `--cache-to` — đặc biệt quan trọng vì `dotnet restore` chậm hơn npm install
3. Chạy test song song: `dotnet test --parallel` (default trong xUnit/NUnit)
4. Chỉ build service bị thay đổi (monorepo): path filters trong GitHub Actions
...
Container chạy non-root là yêu cầu tối thiểu. Trong Dockerfile .NET:
...
│
├── backend/
│   ├── Dockerfile                     # Multi-stage: build (sdk) + runtime (aspnet alpine)
│   ├── ShopLite.Backend.sln           # Solution file
│   ├── src/
│   │   ├── ShopLite.API/
│   │   │   ├── ShopLite.API.csproj
│   │   │   ├── Program.cs             # Minimal API / WebApplication builder
│   │   │   ├── Controllers/
│   │   │   ├── Models/
│   │   │   └── appsettings.json       # Non-secret config (URLs, feature flags)
│   │   └── ShopLite.Application/      # Business logic layer (optional)
│   │       └── ShopLite.Application.csproj
│   └── tests/
│       ├── ShopLite.UnitTests/
│       │   └── ShopLite.UnitTests.csproj
│       └── ShopLite.IntegrationTests/ # Hit real PostgreSQL via Testcontainers
│           └── ShopLite.IntegrationTests.csproj
│
├── nginx/
...
| Backend | .NET 8, ASP.NET Core | Performance cao, strongly typed, ecosystem enterprise |
| Database | PostgreSQL 15 (EF Core + Npgsql) | ACID compliance, ORM tích hợp tốt |
| Cache | Redis 7 (StackExchange.Redis) | Session storage, distributed cache, rate limiting |
...
CI workflow trigger khi có push lên bất kỳ branch nào hoặc khi mở PR. Jobs: checkout code, setup .NET 8 SDK, restore NuGet packages (với `dotnet restore`), chạy `dotnet format --verify-no-changes` để lint/format check, chạy `dotnet test` cho unit tests và integration tests. Integration tests dùng Testcontainers để spin up PostgreSQL và Redis container thật trong pipeline — không mock. Job này cần chạy dưới 5 phút; cache NuGet bằng `actions/cache` với key là hash của `*.csproj` files.

CD workflow trigger khi CI pass trên branch main, và cần manual approval từ team lead. Steps: build Docker image với multi-stage build (.NET SDK build → ASP.NET runtime), tag với git SHA ngắn (7 ký tự), push lên GitHub Container Registry, SSH vào EC2 production server, pull image mới, chạy `docker compose up -d --no-deps backend`. Zero-downtime vì Docker Compose health check (`/health` endpoint của ASP.NET) đảm bảo container mới healthy trước khi traffic chuyển sang.
...
Trong ShopLite, tôi dùng EF Core Migrations. Migration được tạo bằng `dotnet ef migrations add <Name>` và apply bằng `dotnet ef database update`. Trong CI/CD, migration chạy trong một step riêng biệt trước khi restart application container — đảm bảo schema ready trước khi app mới start.

```bash
# Tạo migration khi thay đổi model
dotnet ef migrations add AddOrderStatusIndex --project src/ShopLite.API

# Apply migration trong CI/CD (dùng connection string từ Secrets Manager)
...
Trên Kubernetes, tôi dùng init container pattern: init container chạy `dotnet ef database update` với image riêng, chỉ khi exit 0 thì app container mới start. Nếu migration fail, pod ở trạng thái Init:Error và app không receive traffic.
...
**II. Dependencies**: Khai báo explicit trong `.csproj` với version cụ thể, không dùng global packages. `dotnet restore` trong Docker build đảm bảo reproducible — đặc biệt quan trọng vì NuGet packages phải match chính xác.

**III. Config**: Mọi thứ thay đổi giữa environments (DB connection string, API keys) đều là environment variables hoặc `appsettings.{Environment}.json`. ASP.NET Core có built-in `IConfiguration` hierarchy: appsettings → environment variables → command line → User Secrets (local dev).

**IV. Backing Services**: DB (PostgreSQL via EF Core + Npgsql), Redis (StackExchange.Redis), S3 (AWSSDK.S3) được treat như attached resources. Connection string trong env var — có thể swap từ local PostgreSQL sang RDS chỉ bằng đổi `ConnectionStrings__DefaultConnection`.

**VII. Port Binding**: ASP.NET Core app bind port qua `ASPNETCORE_URLS=http://+:8080` — không cần IIS hay Apache làm webserver riêng (Kestrel là built-in webserver).

**IX. Disposability**: Fast startup (< 5s với native AOT, < 10s standard), graceful shutdown khi nhận SIGTERM: ASP.NET Core có built-in `IHostedService.StopAsync`, `IApplicationLifetime` để handle inflight requests và close DB connections.

**XI. Logs**: Dùng `ILogger<T>` interface, output stdout via `ConsoleLogger`. Container runtime thu thập và ship qua Promtail. Không bao giờ write logs ra file trong container. Structured logging với Serilog: `Log.Information("Order {OrderId} created by {UserId}", orderId, userId)` → queryable trong Loki.
...
A: Multi-stage build tách biệt build environment (cần SDK, compiler, dev tools) khỏi runtime environment (chỉ cần compiled binary/DLLs).

Ví dụ ShopLite backend .NET:

```dockerfile
# Stage 1: Restore dependencies (tận dụng layer cache)
...
# Stage 3: Runtime — chỉ cần ASP.NET runtime, không cần SDK
FROM mcr.microsoft.com/dotnet/aspnet:8.0-alpine AS runtime
WORKDIR /app
...
Kết quả: image từ ~800MB (full .NET SDK + source + obj/bin) xuống ~120MB (ASP.NET alpine runtime + published DLLs). Nhỏ hơn = pull nhanh hơn khi deploy = deploy nhanh hơn.

Security benefit: không có .NET SDK, MSBuild, NuGet CLI trong runtime image. SDK image chứa nhiều package hơn → nhiều CVEs tiềm ẩn hơn. Trivy scan report trên `aspnet:8.0-alpine` sạch hơn nhiều so với `sdk:8.0`.

Cache benefit: Copy `*.csproj` trước, `dotnet restore` sau — riêng một stage `restore`. Nếu source code thay đổi nhưng `.csproj` không đổi, Docker dùng lại cached layer của `dotnet restore` — tiết kiệm 30-60 giây mỗi build (NuGet restore chậm hơn npm install đáng kể).
...
Chuyển sang GitHub Actions tab: "CI đang chạy. Bạn có thể thấy 3 jobs song song: lint-format (dotnet format --verify-no-changes), unit-tests, và integration-tests. Integration tests dùng Testcontainers để spin up PostgreSQL thật trong pipeline — không mock DB."
...
Click Approve. "Docker image đang được build với multi-stage .NET build — stage 1 dùng `dotnet/sdk:8.0-alpine` để build, stage 2 chỉ dùng `dotnet/aspnet:8.0-alpine` runtime. Tag là git SHA ngắn — `abc1234`. Push lên GHCR. Sau đó SSH vào EC2 và chạy `docker compose pull && docker compose up -d`."

Show deployment succeeds: "Deploy xong. Zero-downtime vì Docker Compose health check — container mới phải pass `GET /health` (ASP.NET Core built-in health check endpoint) trước khi cũ bị stop."
...
Microservices patterns: Circuit breaker (Polly v8 — built-in với .NET 8, `AddResiliencePipeline`), Saga pattern cho distributed transactions, Event sourcing basics, Message queues (SQS, RabbitMQ, MassTransit trên .NET).
...
**Học được:**
- Dockerfile best practices: layer caching, multi-stage builds (.NET SDK → ASP.NET runtime), non-root user
- Docker networking: bridge, host, overlay networks
- Volume management: bind mounts vs named volumes

**Khó khăn gặp phải:**
- ASP.NET Core app in container không connect được tới PostgreSQL
- "Connection refused to localhost:5432" — mất 1 tiếng debug
- `dotnet restore` chậm vì không có layer cache — mỗi build mất 3 phút
...
- Tách stage `COPY *.csproj → dotnet restore` riêng trước khi `COPY . .` → layer cache hoạt động đúng, build còn 30 giây
- Lesson learned: network isolation là feature, không phải bug

**Insight quan trọng:**
- Docker layer caching với .NET: `COPY *.csproj` trước `dotnet restore`, sau đó mới `COPY . .` và `dotnet publish`. Thứ tự này tiết kiệm 2-3 phút mỗi build khi chỉ thay đổi source code.
- Image size: `dotnet/sdk:8.0` (~800MB) vs `dotnet/aspnet:8.0-alpine` (~120MB). Multi-stage là bắt buộc cho .NET production images.

**Sẽ dùng khi phỏng vấn khi được hỏi về:**
- "Container của bạn không connect được DB, debug như thế nào?"
- "Tối ưu Docker image .NET như thế nào?"
```
